...
def augment(batch):
	...
	if random.random() < .5:
		if random.random() < .5:
			images = torchvision.transforms.functional.rotate(images, 5, torchvision.transforms.InterpolationMode.BILINEAR, False)
			labels = torchvision.transforms.functional.rotate(labels, 5, torchvision.transforms.InterpolationMode.NEAREST, False)
		else:
			images = torchvision.transforms.functional.rotate(images, -5, torchvision.transforms.InterpolationMode.BILINEAR, False)
			labels = torchvision.transforms.functional.rotate(labels, -5, torchvision.transforms.InterpolationMode.NEAREST, False)
...
class Dataset:
	def __init__(this, path_to_gt, gt_to_image_path, start, end, batchSize, shuffle, map_ids, augment):
