...
writer = SummaryWriter("runs/augmented_iteration_1")
...
maskScale = 16
maskBlur = 95
cowmask = Cowmask(batchSize, height, width, maskScale, maskBlur)

cross_entropy = torch.nn.CrossEntropyLoss(reduction='mean', ignore_index=ignore_index)
...
while epoch < epochs:
	...
	while True:
		...
		input, target = cowmask.mix(input, target)
