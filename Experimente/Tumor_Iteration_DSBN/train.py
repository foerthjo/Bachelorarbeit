...
writer = SummaryWriter("runs/augmented_iteration_2_1_" + str(datetime.now()).replace(':', '_'))
...
cleanModel.to(device)
augmentedModel.to(device)
...
while epoch < epochs:
	...
	while True:
		...
		with torch.no_grad():
			...
			confidence = torch.gather(softmax(cleanModel(mixedInput)), dim=1, index=index).view([halfBatchSize, height, width])

		cleanOptimizer.zero_grad()
		augmentedOptimizer.zero_grad()

		cleanOutput = cleanModel(cleanInput)
		mixedOutput = augmentedModel(mixedInput)
