...
class Cowmask:
	...
	def newMask(this):
		...
		imageMask = noise.view([1, 1, this.height, this.width]).expand([this.halfBatchSize, 4, this.height, this.width])
		labelMask = noise.view([1, this.height, this.width]).expand([this.halfBatchSize, this.height, this.width])
		return imageMask, labelMask
	
	def mix(this, images, labels, confidence):
		...
		confidence1 = confidence[0:this.halfBatchSize]
		confidence2 = confidence[this.halfBatchSize:this.batchSize]
		...
		confidence = torch.cat([confidence1 * labelMask + confidence2 * labelMaskInv, confidence1 * labelMaskInv + confidence2 * labelMask], dim=0)

		return images, labels, confidence