...
def drawBox(tensor, box, value):
	...
	channels, h, w = tensor.size()

	column = torch.arange(0, h, device=device).view([h, 1])
	row = torch.arange(0, w, device=device).view([1, w])
	...
	box_mask = box_mask.type(torch.bool).view([1, h, w])
...
for path in gtPaths:
	...
	boxgt_mask_1 = torch.zeros((1, height, width), device=device, dtype=torch.long)
	boxgt_mask_2 = torch.zeros((1, height, width), device=device, dtype=torch.long)

	if not box1 == None:
		drawBox(boxgt_inner, box1, ignore_index)
		drawBox(boxgt_inner, scaleBoxDown(box1), 1)
		drawBox(boxgt_mask_1, box1, 1)
	
	if not box2 == None:
		drawBox(boxgt_inner, box2, ignore_index)
		drawBox(boxgt_inner, scaleBoxDown(box2), 2)
		drawBox(boxgt_mask_2, box2, 1)

	wrong_mask = torch.zeros((1, height, width), device=device, dtype=torch.uint8)
	wrong_mask += (1 - boxgt_mask_1) * (result == 1).type(torch.uint8)
	wrong_mask += (1 - boxgt_mask_2) * (result == 2).type(torch.uint8)
