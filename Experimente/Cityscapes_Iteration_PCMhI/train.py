...
writer = SummaryWriter("runs/iteration_" + str(datetime.now()).replace(':', '_'))
...
gt_to_box_path = lambda gt: gt.replace("iteration_postpro_1\\labels_", "boxes_i\\boxes_")
start = 0
end = 1
shuffle = True
map_ids = False
dataset = Dataset(path_to_gt, gt_to_image_path, gt_to_box_path, start, end, batchSize, shuffle, map_ids)
...
while epoch < epochs:
	...
	while True:
		...
		input, target, pseudolabelMask = data
		if input.shape[0] != batchSize or target.shape[0] != batchSize or pseudolabelMask.shape[0] != batchSize:
			break
		
		if random.random() < .5:
			input = input.flip([3])
			target = target.flip([2])
			pseudolabelMask = pseudolabelMask.flip([2])

		cleanInput, cleanTarget, cleanPseudolabelMask = input[0:halfBatchSize], target[0:halfBatchSize], pseudolabelMask[0:halfBatchSize]
		mixedInput, mixedTarget = input[halfBatchSize:batchSize], target[halfBatchSize:batchSize]

		cleanTarget.masked_fill_(cleanPseudolabelMask, ignore_index)
