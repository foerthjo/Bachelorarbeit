...
from LabelIDs import nameToLabelID, trainIDsByLabelID, ignore_index
import numpy as np
import json
...
def scaleBoxDown(box):
	border = min(box["width"] * .25, box["height"] * .25)
	box["x"] += border
	box["y"] += border
	box["endX"] -= border
	box["endY"] -= border
...
def loadGT(path):
	...
	for box in boxes:
		if not box["trainID"] == 0:
			drawBox(labels, box, ignore_index)
...
for path in gtPaths:
	gt = loadGT(path)
	write_png(gt.view([1, targetHeight, targetWidth]).type(torch.uint8).to(cpu), "G:\\cityscapes_dataset\\train\\boxes_i\\boxes_" + str(imgIndex) + ".png", 9)
	print(str(imgIndex))
	imgIndex += 1